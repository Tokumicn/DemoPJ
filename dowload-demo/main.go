...
func NewFileDownloader(url, outputFileName, outputDir string, totalPart int) *FileDownloader {
	...
	return &FileDownloader{
		...
		outputDir:      outputDir,
		totalPart:      totalPart,
		doneFileParts:  make([]filePart, totalPart),
	}
}
...
func (d *FileDownloader) head() (int, error) {
	...
	if resp.Header.Get("Accept-Ranges") != "bytes" {
		return 0, errors.New("服务器不支持文件端点续传")
	}
...
func (d *FileDownloader) Run() error {
	...
	eachSize := fileTotalSize / d.totalPart // 每个文件块的大小
...
func (d *FileDownloader) mergeFileParts() error {
	...
	hash := sha256.New()
	totalSize := 0
	for _, s := range d.doneFileParts {
		mergedFile.Write(s.Data)
		hash.Write(s.Data)
		totalSize += len(s.Data)
	}
	if totalSize != d.fileSize {
		return errors.New("文件下载不完整")
	}
